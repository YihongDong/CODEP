...
class VocabgrammarEntry(VocabEntry):
    def __init__(self):
        super(VocabgrammarEntry, self).__init__()
        self.transition2id = {}
        self.key2id = {}
    ...
    def add(self, word):
        if word not in self:
            wid = self.word2id[word] = len(self)
            self.id2word[wid] = word
            self.key2id[word[0]] = wid
            if word[1] not in self.transition2id.keys():
                self.transition2id[word[1]] = [wid] 
            else:
                self.transition2id[word[1]].append(wid)
            return wid
        else:
            self.transition2id[word[1]].append(wid)
            return self[word]
