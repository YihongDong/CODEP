...
from .generation_beam_constraints import Constraint, DisjunctiveConstraint, PhrasalConstraint, ConstraintListState
...
class GenerationMixin:
    ...
    def make_constraint_states(self, n, constraints):
        return [ConstraintListState([constraint.copy() for constraint in constraints]) for _ in range(n)]
    ...
    def sample(
        ...
        **model_kwargs,
        ...
        pda_list = [PDA() for _ in range(orig_len)]
        unfinished_tokens = [[] for _ in range(orig_len)]
        token_len = [1] * orig_len
        pda_first_update = 1
        num_state = [0] * orig_len
        name_state = [0] * orig_len
        string_state = [0] * orig_len
        string_quotation = [0] * orig_len
        num_dot = [0] * orig_len
        ...
        while True:
            ...
            for idx in range(orig_len):
                pda = pda_list[idx]
                if pda_first_update:
                    token = tokenize(code = text, version_info = parse_version_string('3.7'))
                    for one in token:
                        print(one.type)
                        if one.type != DEDENT and one.type != ENDMARKER:
                            print("###############")
                            print(one)
                            print(one.string)
                            pda.update(one)
                if len(unfinished_tokens[idx]) != 0:
                    ...
                    for one in unfinished_tokens[idx]:
                        ...
                        if len(one) > 1:
                            # one.pop(0)
                            temp_tokens.append(one[1:])   #todo:测试
                        print(temp_tokens)
                ...
                #         raise ValueError("If `eos_token_id` is defined, make sure that `pad_token_id` is defined.")
                #     next_tokens = next_tokens * unfinished_sequences + pad_token_id * (1 - unfinished_sequences)
                if [next_token.item()] in advance_tokens:
                    unfinished_tokens[idx] = []
                    full_token = next_token
                    if token_len[idx] > 1 and constraint_state[idx] == 0:
                        ...
                        if [next_token.item()] in first_nametokensid and num_state[idx] == 0 and string_state[idx] == 0 and name_state[idx] == 0 and constraint_state[idx] == 0:
                            ...
                            if string_state[idx] == -1:
                                ...
                                string_state[idx] = 1
                            if [next_token.item()] == [1] or [next_token.item()] == [366]:
                                string_quotation[idx] = 1
                            elif [next_token.item()] == [6] or [next_token.item()] == [705]:
                                string_quotation[idx] = 2
                            elif [next_token.item()] == [2]:
                                string_quotation[idx] = 3
                            elif [next_token.item()] == [10786] or [next_token.item()] == [19203]:
                                string_quotation[idx] = 4
                                token_len[idx] = name_state[idx] + num_state[idx] + string_state[idx]
                                name_state[idx] = 0
                                num_state[idx] = 0
                                num_dot[idx] = 0
                                ...
                                code = ''
                                if token_len[idx] > 1:
                                    full_token = torch.cat([input_ids[idx,-token_len[idx]+1:]])
                                    print(full_token)
                                    code = tokenizer.decode(full_token)
                                token = tokenize(code = code + '(', version_info = parse_version_string('3.7'))
                                token_len[idx] = 1
                            ...
                            if [next_token.item()] == [13]:
                                num_dot[idx] = 1
                            num_state[idx] += 1
                        elif [next_token.item()] in last_numtokensid2 and name_state[idx] == 0 and string_state[idx] == 0 and num_state[idx] >= 1 and num_dot[idx] == 1 and constraint_state[idx] == 0:
                            num_state[idx] += 1
                        elif [next_token.item()] in last_stringtokensid and name_state[idx] ==0 and num_state[idx] == 0 and string_state[idx] >= 1 and constraint_state[idx] == 0:
                            string_state[idx] += 1
                            ...
                            if ((string_quotation[idx] == 1 or string_quotation[idx] == 6 or string_quotation[idx] == 7) and ('"' in tokenizer.decode([next_token.item()]) and tokenizer.decode([next_token.item()]) != '\\"'))  or ((string_quotation[idx] == 2 or string_quotation[idx] == 4 or string_quotation[idx] == 5) and ("'" in tokenizer.decode([next_token.item()]) and tokenizer.decode([next_token.item()]) != "\\'")):
                                ...
                                print(code)
                                token_len[idx] = 1
                                if string_quotation[idx] == 4 or string_quotation[idx] == 5 or string_quotation[idx] == 6 or string_quotation[idx] == 7:
                                    code = code[1:]
                                    print("6666666666")
                                    print(code)
                                token = tokenize(code = code, version_info = parse_version_string('3.7'))
                ...
                if not def_nextokens: #判断next_tokens是否定义
                    next_tokens = torch.cat([next_tokens, next_token],dim=0)
                if def_nextokens:
...
import os
import copy
from transformers import GPT2Tokenizer, CodeGenTokenizer
import keyword
import re
...
def load_grammar(version= None, path= None):
    # /home/liuyc/PDAConstraint/grammar/grammar37.txt
    version_info = parse_version_string(version)
    file = os.path.join('../../../../../../../PDAConstraint/grammar','grammar%s%s.txt' % (version_info.major, version_info.minor))
...
mykeyword.extend(tokenizer.tokenize(' '+kw) for kw in keyword.kwlist)
for kw in mykeyword:
    final_mykeyword.append(kw[0])
for one in tokenizer.get_vocab().keys():
    if (re.match("^Ġ[A-Za-z_]+$", one) or re.match("^[A-Za-z_]+$", one)) and one not in final_mykeyword:
        first_nametokensid.append([tokenizer.get_vocab().get(one)])
    if re.match("^[A-Za-z0-9_]+$", one):
        last_nametokensid.append([tokenizer.get_vocab().get(one)])
    if re.match("^Ġ?[0-9]+$", one):
        first_numtokensid.append([tokenizer.get_vocab().get(one)])
    if re.match("^[0-9]+$", one) or re.match("^\.$", one):
        last_numtokensid1.append([tokenizer.get_vocab().get(one)])
    if re.match("^[0-9]+$", one):
        last_numtokensid2.append([tokenizer.get_vocab().get(one)])
    if re.match('^."$', one) or re.match("^.'$", one) or re.match('^Ġ."$', one) or re.match("^Ġ.'$", one) or re.match('^".$', one) or re.match("^'.$", one) or re.match('^Ġ".$', one) or re.match("^Ġ'.$", one):
...
class PDA:
    def __init__(self):
        self.PDA = PyParser(grammar._pgen_grammar, error_recovery=False, start_nonterminal=grammar._start_nonterminal)
    ...
    def advance(self, num_state, name_state, string_state, num_dot, indent_state, constraint_state, multistring_state):
        ...
        reserved_action_types = set(one.value for one in action_types if one.__class__.__name__ != 'PythonTokenTypes')
        reserved_action_types = sorted(reserved_action_types)
        pytoken_action_types = set(one.value.name for one in action_types if one.__class__.__name__ == 'PythonTokenTypes')
        ...
        if num_state == 0 and name_state == 0 and string_state <= 0 and indent_state == 0 and constraint_state == 0:
            for one in reserved_action_types:
                if one == ":":
                    temp_PDA = self.PDA.copy()
                    token = tokenize(code = ':', version_info = parse_version_string('3.7'))
                    for tmp_one in token:
                            if tmp_one.string != '':
                                temp_PDA._add_token(tmp_one)
                    action_types = set(temp_PDA.stack[-1].dfa.transitions.keys())
                    while temp_PDA.stack[-1].dfa.is_final:
                        temp_PDA.stack.pop()
                        action_types = action_types | set(temp_PDA.stack[-1].dfa.transitions.keys())
                    temp_reserved_action_types = set(tmp_one.value for tmp_one in action_types if tmp_one.__class__.__name__ != 'PythonTokenTypes')
                    if ")" in temp_reserved_action_types:
                ...
                if one == "[":
                    ...
                    while temp_PDA.stack[-1].dfa.is_final:
                        ...
                        action_types = action_types | set(temp_PDA.stack[-1].dfa.transitions.keys())
                    temp_reserved_action_types = set(tmp_one.value for tmp_one in action_types if tmp_one.__class__.__name__ != 'PythonTokenTypes')
                    temp_pytoken_action_types = set(tmp_one.value.name for tmp_one in action_types if tmp_one.__class__.__name__ == 'PythonTokenTypes')
                    if "]" in temp_reserved_action_types:
                        force_flexible_ids.append(tokenizer.encode('[]'))
                        force_flexible_ids.append(tokenizer.encode(' '+'[]'))
                    if "[" in temp_reserved_action_types:
                        force_flexible_ids.append(tokenizer.encode('[['))
                        force_flexible_ids.append(tokenizer.encode(' '+'[['))
                    if "STRING" in temp_pytoken_action_types:
                        force_flexible_ids.append(tokenizer.encode('["'))
                        force_flexible_ids.append(tokenizer.encode(' '+'["'))
                        force_flexible_ids.append(tokenizer.encode("['"))
                        force_flexible_ids.append(tokenizer.encode(" "+"['"))
                ...
                if one == "(":
                    ...
                    if ")" in temp_reserved_action_types:
                        force_flexible_ids.append(tokenizer.encode('()'))
                        force_flexible_ids.append(tokenizer.encode(' '+'()'))
                        new_temp_PDA = self.PDA.copy()
                        token = tokenize(code = '()', version_info = parse_version_string('3.7'))
                        for tmp_one in token:
                            if tmp_one.string != '':
                                new_temp_PDA._add_token(tmp_one)
                        action_types = set(new_temp_PDA.stack[-1].dfa.transitions.keys())
                        while new_temp_PDA.stack[-1].dfa.is_final:
                            new_temp_PDA.stack.pop()
                            action_types = action_types | set(new_temp_PDA.stack[-1].dfa.transitions.keys())
                        new_temp_reserved_action_types = set(tmp_one.value for tmp_one in action_types if tmp_one.__class__.__name__ != 'PythonTokenTypes')
                        if ")" in new_temp_reserved_action_types:
                            force_flexible_ids.append(tokenizer.encode('())'))
                            force_flexible_ids.append(tokenizer.encode(' '+'())'))
                    if "STRING" in temp_pytoken_action_types:
                        force_flexible_ids.append(tokenizer.encode('("'))
                ...
                if one == "{":
                    ...
                    if "}" in temp_reserved_action_types:
                        force_flexible_ids.append(tokenizer.encode(' '+'{}'))
                force_flexible_ids.append(tokenizer.encode(' '+one))
                force_flexible_ids.append(tokenizer.encode(one))
            for one in pytoken_action_types:
                if one == 'NUMBER':
                    force_flexible_ids.extend(first_numtokensid)
                if one == 'NAME':
                    force_flexible_ids.extend(first_nametokensid)
                if one == 'STRING':
                    force_flexible_ids.extend(first_stringtokensid)
                if one == 'NEWLINE':
                    force_flexible_ids.append(tokenizer.encode("\n"))
                if one == 'INDENT':
                    force_flexible_ids.append(tokenizer.encode("\t"))
            ...
            if name_state == 1:
                ...
                for one in token:
                    if one.string != '':
                        new_PDA._add_token(one)
                action_types = set(new_PDA.stack[-1].dfa.transitions.keys())
                while new_PDA.stack[-1].dfa.is_final:
                    new_PDA.stack.pop()
                    action_types = action_types | set(new_PDA.stack[-1].dfa.transitions.keys())
                reserved_action_types = set(one.value for one in action_types if one.__class__.__name__ != 'PythonTokenTypes')
                reserved_action_types = sorted(reserved_action_types)
                pytoken_action_types = set(one.value.name for one in action_types if one.__class__.__name__ == 'PythonTokenTypes')
                pytoken_action_types = sorted(pytoken_action_types)
                self.reserved_action_types = reserved_action_types
                self.pytoken_action_types = pytoken_action_types
            force_flexible_ids = []
            for one in self.reserved_action_types:
                if one == ":":
                    temp_PDA = self.PDA.copy()
                    token = tokenize(code = 'a:', version_info = parse_version_string('3.7'))
                    for tmp_one in token:
                    ...
                    while temp_PDA.stack[-1].dfa.is_final:
                        temp_PDA.stack.pop()
                        action_types = action_types | set(temp_PDA.stack[-1].dfa.transitions.keys())
                    reserved_action_types = set(tmp_one.value for tmp_one in action_types if tmp_one.__class__.__name__ != 'PythonTokenTypes')
                    if ")" in reserved_action_types:
                        force_flexible_ids.append(tokenizer.encode(':)'))
                        force_flexible_ids.append(tokenizer.encode(' '+':)'))
                    if "]" in reserved_action_types:
                        force_flexible_ids.append(tokenizer.encode(':]'))
                ...
                if one == "[":
                    ...
                    while temp_PDA.stack[-1].dfa.is_final:
                        ...
                        action_types = action_types | set(temp_PDA.stack[-1].dfa.transitions.keys())
                    reserved_action_types = set(tmp_one.value for tmp_one in action_types if tmp_one.__class__.__name__ != 'PythonTokenTypes')
                    pytoken_action_types = set(tmp_one.value.name for tmp_one in action_types if tmp_one.__class__.__name__ == 'PythonTokenTypes')
                    if "]" in reserved_action_types:
                        force_flexible_ids.append(tokenizer.encode('[]'))
                        force_flexible_ids.append(tokenizer.encode(' '+'[]'))
                    if "[" in reserved_action_types:
                        force_flexible_ids.append(tokenizer.encode('[['))
                        force_flexible_ids.append(tokenizer.encode(' '+'[['))
                    if "STRING" in pytoken_action_types:
                        force_flexible_ids.append(tokenizer.encode('["'))
                        force_flexible_ids.append(tokenizer.encode(' '+'["'))
                        force_flexible_ids.append(tokenizer.encode("['"))
                        force_flexible_ids.append(tokenizer.encode(" "+"['"))
                ...
                force_flexible_ids.append(tokenizer.encode(' '+one))
                force_flexible_ids.append(tokenizer.encode(one))
            for one in self.pytoken_action_types:
                if one == 'NEWLINE':
                    force_flexible_ids.append(tokenizer.encode("\n"))
            force_flexible_ids.extend(last_nametokensid)
        elif string_state >= 1 and constraint_state == 0:
            if string_state == 1 or string_state == 2:
                new_PDA = self.PDA.copy()
                token = tokenize(code = '"a"', version_info = parse_version_string('3.7'))
                for one in token:
                        if one.string != '':
                ...
                self.pytoken_action_types = pytoken_action_types
            force_flexible_ids = []
            force_flexible_ids.extend(last_stringtokensid)
            ...
            if num_state == 1:
                new_PDA = self.PDA.copy()
                token = tokenize(code = '1', version_info = parse_version_string('3.7'))
                for one in token:
                        if one.string != '':
                            new_PDA._add_token(one)
                action_types = set(new_PDA.stack[-1].dfa.transitions.keys())
            ...
            for one in self.pytoken_action_types:
                if one == 'NEWLINE':
                    force_flexible_ids.append(tokenizer.encode("\n"))
            if num_dot == 1:
                force_flexible_ids.extend(last_numtokensid2)
            else: 
            ...
            for one in reserved_action_types:
                ...
                force_flexible_ids.append(tokenizer.encode(' '+one))
                force_flexible_ids.append(tokenizer.encode(one))
            # for indent_num in range(1,self.indent+2):
            #     force_flexible_ids.append(tokenizer.encode("\t"*indent_num))
        
        elif constraint_state == 1:
            force_flexible_ids.extend(last_stringtokensid)

        return force_flexible_ids
    
    def update(self, token):
        self.PDA._add_token(token)
        if token.type.name == 'INDENT':
            self.indent = self.indent + 1
        elif token.type.name == 'DEDENT':
            self.indent = self.indent - 1
    
    def copy(self):
        new_PDA = PDA()
        new_PDA.PDA = self.PDA.copy()
        new_PDA.indent = self.indent
        return new_PDA
